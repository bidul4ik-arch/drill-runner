...
func run() -> void:
	...
	for asset in ["mining_robot","crystal_guardian","giant_drill"]:
		add_model(asset,Vector3.ZERO,-.22)
		camera.position=Vector3(8,5.4,13.5);camera.look_at(Vector3(0,2.3,0));camera.fov=36
		await shot(asset)
		for a in actors:a.queue_free()
		actors.clear();await get_tree().process_frame
	print("ART REVIEW CAPTURED")
	get_tree().quit()
