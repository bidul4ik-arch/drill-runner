...
func run() -> void:
	...
	for i in 3:add_model("explorer",Vector3((i-1)*1.45,0,0),[PI,PI*.5,0][i])
	await shot("hero-turnaround")
	for a in actors:a.queue_free()
	actors.clear();await get_tree().process_frame
	var clips: Array=["idle","run","sprint","jump","fall","land","slide","lane_left","lane_right","boost","hit","victory"]
	for page in 4:
		for i in 3:
			var clip: String=clips[page*3+i]
			var model:=add_model("explorer",Vector3((i-1)*1.45,0,0),-.35)
			Profile.apply_skin(model,"explorer")
			var anim: AnimationPlayer=model.find_child("AnimationPlayer",true,false)
			anim.play(clip);anim.seek(.40 if clip in ["slide","hit","victory"] else anim.current_animation_length*.32,true);anim.pause()
		camera.position=Vector3(0,2.3,7.8);camera.look_at(Vector3(0,1.0,0));camera.fov=33
		await shot("poses-%d" % page)
		for a in actors:a.queue_free()
		actors.clear();await get_tree().process_frame
	print("HERO MOTION REVIEW CAPTURED")
	get_tree().quit()
